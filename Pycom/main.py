...
def receive_data():
    ...
    print('receive_data()')
    data = ""
    joiner.send_data_bytes(bytes([1])) # makes uplink, necessary to make an downlink after
    data = joiner.receive_data_blocking() # receive the latest downlink that put in our applciation
    print('data: ', data)

def encodePayloadWithCharacIdentifier(value, id):
...
CHARACTERISTIC_OBJECT_ID = 0x2AC3 # Object ID
...
ID_USERDATA_STRING = [0x1]
ID_BATTERY_LEVEL = [0x2]
ID_LOCATION_LATITUDE = [0x3]
ID_LOCATION_LONGITUDE = [0x4]
ID_PHONE_ID = [0x5]
ID_BROADCAST_STRING = [0x6]
...
def ble_thread():
    ...
    def char1_cb_handler(chr, data): # USER_DATA -> ObjectName
        ...
        if  events & Bluetooth.CHAR_WRITE_EVENT:
            finalValue = encodePayloadWithCharacIdentifier(value, ID_USERDATA_STRING)
            print("On char 1 Write request with value = {}".format(finalValue))
            _thread.start_new_thread(send_data, (finalValue,)) # must have ','... https://stackoverflow.com/a/37116824
        else:
    ...
    def char5_cb_handler(chr, data): # PHONE -> OBJECT ID
        ...
        if  events & Bluetooth.CHAR_WRITE_EVENT:
            finalValue = encodePayloadWithCharacIdentifier(value, ID_PHONE_ID)
            print("On char 5 Write request with value = {}".format(finalValue))
            _thread.start_new_thread(send_data, (finalValue,))
        else:
    ...
    def char6_cb_handler(chr, data): # PUBLIC_BROADCAST -> STRING
        ...
        if  events & Bluetooth.CHAR_WRITE_EVENT:
            finalValue = encodePayloadWithCharacIdentifier(value, ID_BROADCAST_STRING)
            print("On char 6 Write request with value = {}".format(finalValue))
            _thread.start_new_thread(send_data, (finalValue,))
        else:
